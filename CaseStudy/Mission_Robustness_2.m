function [negative_rob,xx,yy,zz] = Mission_Robustness_1(var,optParams)
%%
import casadi.*
...
for d = 1:optParams.N_drones
    ...
    if size(drone_goals)
        for g = drone_goals(:,1)'
            ...
            I{i,1}=[drone_goals(i,2) drone_goals(i,3)];
            I{i,2}=[1+drone_goals(i,2)*N_per_T/T:1+drone_goals(i,3)*N_per_T/T];
            i = i + 1;
        end
            rho = [rho; robustness_goal_ea_al(xx,yy,zz,d,g,I,optParams)];
    end
