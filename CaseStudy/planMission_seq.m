function [w_opt, optParams, time_taken] = planMission_seq(missionHandle)
import casadi.*
...
disp(['Formulating in Casadi for drone',num2str(d),'...']);
...
% Replicate Constraints for all drones
    ...
    for k = 1:H_formula
        ...
        px_k = w(k*3+1);px_km1 = w((k-1)*3+1);
        py_k = w(k*3+2);py_km1 = w((k-1)*3+2);
        pz_k = w(k*3+3);pz_km1 = w((k-1)*3+3);
        % dv for all axes
        
        dv_x = v(k*3+1) - v((k-1)*3+1);
        dv_y = v(k*3+2) - v((k-1)*3+2);
        dv_z = v(k*3+3) - v((k-1)*3+3);
        
        vx_k = v(k*3+1);vx_km1 = v((k-1)*3+1);
        vy_k = v(k*3+2);vy_km1 = v((k-1)*3+2);
        vz_k = v(k*3+3);vz_km1 = v((k-1)*3+3);
...
prob = struct('f', Mission_Robustness_seq(var,optParams,d,w_opt_rob), 'x', var, 'g', g);
%get the solver to solve the problem
...
%for i = 1:N_drones
    mission_type = 1*rem(d,2) + 2*(rem(d,2)==0);
    optParams.V_bounds = V_bounds{1};
    [temp, tempv] = Mission_Get_Initial_Waypoints([p0(:,d);v0(:,d)],...
    ...
    w0 = temp;
    vv0 = tempv;
    pos0 = reshape(temp,3,H_formula+1);
    vel0 = reshape(tempv,3,H_formula+1);
%end
optParams.V_bounds = V_bounds;
var0 = [w0;vv0];
optParams.var0 = var0;
Init_waypoints_rob = Mission_Robustness_seq(var0,optParams,d,w_opt_rob);
toc
...
w_opt_temp = full(sol.x);
w_opt_rob=[w_opt_rob,w_opt_temp];
%position
w_opt_w = [w_opt_w; w_opt_temp(1:numel(w_opt_temp)/2)];
%velocity
w_opt_v = [w_opt_v; w_opt_temp(numel(w_opt_temp)/2+1:end)];

end
disp(['Total time taken in solving:',num2str(time_taken),'...']);
w_opt=[w_opt_w; w_opt_v];
end