function [ rho_unsafe ] =robustness_unsafe_seq(xx,yy,zz,d,optParams)
%%
import casadi.*
...
%   Detailed explanation goes here
  ...
  % always not unsafe in x y z
    rho_lb_xx = xx(:,1)-optParams.obs_lb_N{j}(:,1);
    rho_ub_xx = optParams.obs_ub_N{j}(:,1)-xx(:,1);
    %rho_xx = SmoothMin([rho_lb_xx;rho_ub_xx],C);
    
    rho_lb_yy = yy(:,1)-optParams.obs_lb_N{j}(:,2);
    rho_ub_yy = optParams.obs_ub_N{j}(:,2)-yy(:,1);
    %rho_yy = SmoothMin([rho_lb_yy;rho_ub_yy],C);
    
    rho_lb_zz = zz(:,1)-optParams.obs_lb_N{j}(:,3);
    rho_ub_zz = optParams.obs_ub_N{j}(:,3)-zz(:,1);
